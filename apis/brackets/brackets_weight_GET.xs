// Get full bracket view for a weight class including matches, wrestlers, and the current user's picks.
query "brackets/tournament/{id}/weight/{weight}" verb=GET {
  ...
  input {
    ...
    // Weight class in lbs (125, 133, 141, 149, 157, 165, 174, 184, 197, 285)
    int weight
  }
        ...
        foreach ($picks) {
          each as $p {
            var.update $picks_map {
              value = $picks_map
                |set:$p.bracket_match_id:$p.picked_wrestler_id
            }
          }
        }
      }
