...
- **`db.edit`/`db.add`'s `data =` field must be an inline object literal — `data = $someVar` throws a hard push-time error, `"Invalid kind for data - assign:var"`.** Confirmed 2026-07-24 building `leagues_week_type_PUT.xs`/`admin_board_channels_PUT.xs`. This forces writing out separate inline-literal branches for each combination when a payload is conditionally assembled (e.g. 4 leaf branches for "edit-or-add" × "clear fields or not"). **Exception: `db.patch` DOES accept a variable** (`data = $payload`) — this is the established, already-working pattern in `leagues_id_PATCH.xs` for building a partial-update object dynamically via `|set:` chains across many `if ($input.x != null) { ... }` blocks, then writing it once. Reach for `db.patch` (not `db.edit`) whenever the payload needs to be assembled as a variable first.
- **Bare `now` works fine as a direct field assignment (`reviewed_at: now`) but throws `"Not numeric."` when used in inline arithmetic (`now + 604800000`)** — confirmed 2026-07-24 in `admin_board_post_resolve_POST.xs`. **Fix: assign it to a var first** — `var $now { value = now }` — then do the arithmetic on the variable (`$now + 604800000`), matching the pattern `score_league_weeks.xs` already used. Bare `now` used as a plain value (no math) is safe and used throughout the codebase (e.g. `created_at: now` in `db.add` payloads).
