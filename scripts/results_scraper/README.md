...
Scrapes team-level `EventMatches.jsp` pages from the external results
provider, normalizes bout records, dedupes across team pages (each bout
appears on both opponents' pages), and pushes `external_result_candidate`
dicts to the platform ingestion API
...
- Everything is cached on disk (`--cache-dir`, default `.scrape_cache`). Cached
...
`EventMatches.jsp` requires a live provider session (`twSessionId` +
...
Run from `scripts/results_scraper/` (or reference `event_scraper.py` by path).

### Live results: 2026 NCAA Division I Championships

Season `1560238138`, event `8710102132`, team map in
`../../results_scraper_bundle/ncaa_d1_team_ids_2025_26.csv`.

```bash
# one-shot: fetch -> parse -> push
python event_scraper.py run --season 1560238138 --event 8710102132 ^
    --teams-csv ../../results_scraper_bundle/ncaa_d1_team_ids_2025_26.csv ^
...
python event_scraper.py poll --season 1560238138 --event 8710102132 ^
    --teams-csv ../../results_scraper_bundle/ncaa_d1_team_ids_2025_26.csv ^
    --source-config 1 --api-base https://xhuf-7flt-jytp.n7d.xano.io/api:PBpa1T2y ^
...
```bash
python event_scraper.py fetch --season 1560238138 --event 8710102132 ^
    --teams-csv ../../results_scraper_bundle/ncaa_d1_team_ids_2025_26.csv -v

python event_scraper.py parse --event 8710102132 --cache-dir .scrape_cache ^
...
```bash
python event_scraper.py backfill ^
    --season-index ../../results_scraper_bundle/event_index_2025_26.csv ^
    --teams-csv ../../results_scraper_bundle/ncaa_d1_team_ids_2025_26.csv ^
    --cache-dir .scrape_cache --out-dir backfill_out --only-ncaa --limit 5
```
...
```bash
python event_scraper.py teams --html saved_team_links.html --out team_ids_new.csv ^
    --name "2027 NCAA Division I Championships"
```

Team IDs are season-specific — rebuild the map each season (or use
`resolve_team_ids.py`, which drives the live search UI instead of a saved page).
...
- The parser is heuristic; if the provider changes markup substantially,
  add the new shape to `event_match_parser.py` (the self-test is the regression net).
